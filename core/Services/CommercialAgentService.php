...
class CommercialAgentService
{
    ...
    public static function handleDetailed(string $channel, string $externalId, string $name, string $text, array $meta = []): array
    {
        ...
        $thread = self::captureAndEnrich($thread, $channel, $externalId, $name, $text);
        ...
        if (!$conn) {
            ...
            try {
                ...
                $reply = 'Ahora mismo tengo un inconveniente técnico, pero puedo ayudarte igual. Haz tu diagnóstico aquí: '
    ...
    private static function captureAndEnrich(array $thread, string $channel, string $externalId, string $profileName, string $text): array
    {
